...
The Cloudflare Worker schedules a weekly refresh to rebuild the dataset-backed holiday catalog from Calendarific and Tallyfy. Canada and the Gulf countries use country-level data, while each U.S. state combines a calculated federal-holiday baseline with provider data and a state-specific Calendarific overlay. The calculated baseline is also applied in the browser, keeping future U.S. years usable when the live or cached dataset contains a successful but empty provider response. The latest dataset is stored in KV and served directly from KV, and the browser keeps a cached copy as a fallback if the network request fails.
