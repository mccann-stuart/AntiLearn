import { normalizeCalendarific, normalizeTallyfy, mergeHolidayLists } from './holiday_utils.mjs';
import {
    COUNTRY_LEVEL_DATASET_LOCATIONS,
    US_NATIONAL_BASELINE,
    US_STATE_DATASET_LOCATIONS
} from './dataset_locations.mjs';
import { getUsFederalHolidays } from './us_federal_holidays.mjs';

const CALENDARIFIC_URL = 'https://calendarific.com/api/v2/holidays';
const TALLYFY_URL = 'https://tallyfy.com/national-holidays/api';
const CALENDARIFIC_NATIONAL_TYPES = 'national,religious';
const CALENDARIFIC_LOCAL_TYPES = 'local,religious';
const MIN_CALENDARIFIC_SUCCESS_RATE = 0.9;
const YEARS_AHEAD = 5;
const COUNTRY_LOCATION_CONCURRENCY = 2;
const STATE_LOCATION_CONCURRENCY = 3;
const YEAR_CONCURRENCY = 2;

function redactUrl(urlStr) {
    try {
        const url = new URL(urlStr);
        return url.origin + url.pathname;
    } catch (e) {
        return 'invalid-url';
    }
}

function redactErrorMessage(error, secrets = []) {
    let message;
    try {
        message = typeof error?.message === 'string' ? error.message : String(error);
    } catch (e) {
        message = 'Unknown error';
    }

    let redactedMessage = message.replace(/https?:\/\/[^\s]+/g, match => redactUrl(match));
    for (const secret of secrets) {
        if (secret) {
            redactedMessage = redactedMessage.split(secret).join('[redacted]');
        }
    }
    return redactedMessage;
}


function getLogger(logger) {
    return {
        log: typeof logger?.log === 'function' ? logger.log.bind(logger) : () => {},
        warn: typeof logger?.warn === 'function' ? logger.warn.bind(logger) : () => {},
        error: typeof logger?.error === 'function' ? logger.error.bind(logger) : () => {}
    };
}

async function mapWithConcurrency(limit, items, iteratee) {
    const results = new Array(items.length);
    let nextIndex = 0;

    async function worker() {
        while (true) {
            const index = nextIndex;
            nextIndex += 1;
            if (index >= items.length) return;
            results[index] = await iteratee(items[index], index);
        }
    }

    const workerCount = Math.min(limit, items.length);
    await Promise.all(Array.from({ length: workerCount }, () => worker()));
    return results;
}

function getYearsToFetch() {
    const currentYear = new Date().getUTCFullYear();
    return Array.from({ length: YEARS_AHEAD + 1 }, (_, i) => currentYear + i);
}

async function fetchCalendarificHolidays(fetchJson, apiKey, request, logger = console) {
    const {
        countryCode,
        year,
        location = '',
        types = CALENDARIFIC_NATIONAL_TYPES
    } = request;

    if (!apiKey) return [];

    const url = new URL(CALENDARIFIC_URL);
    url.searchParams.set('api_key', apiKey);
    url.searchParams.set('country', countryCode);
    url.searchParams.set('year', String(year));
    url.searchParams.set('type', types);
    if (location) {
        url.searchParams.set('location', location);
    }

    try {
        const data = await fetchJson(url.toString());
        return normalizeCalendarific(data && data.response ? data.response.holidays : null);
    } catch (error) {
        const safeLogger = getLogger(logger);
        const errorMessage = redactErrorMessage(error, [apiKey]);
        safeLogger.error(
            `Failed to fetch Calendarific holidays from ${redactUrl(url.toString())}: ${errorMessage}`
        );
        throw new Error(`Calendarific fetch failed: ${errorMessage}`);
    }
}

async function fetchTallyfyHolidays(fetchJson, countryCode, year) {
    const url = `${TALLYFY_URL}/${countryCode}/${year}.json`;
    const data = await fetchJson(url);
    return normalizeTallyfy(data ? data.holidays : null);
}

function createDatasetSkeleton(apiKey) {
    const generatedAt = new Date().toISOString();
    return {
        generatedAt,
        updatedAt: generatedAt.slice(0, 10),
        sources: {
            calendarific: {
                enabled: Boolean(apiKey),
                requests: {
                    expected: 0,
                    attempted: 0,
                    succeeded: 0,
                    failed: 0,
                    skipped: 0
                },
                successRate: 0
            },
            tallyfy: {
                enabled: true
            }
        },
        locations: {}
    };
}

function createLocationEntry(location, yearsData) {
    return {
        name: location.label,
        countryCode: location.countryCode,
        years: yearsData
    };
}

function assertCalendarificPublicationSafe(
    dataset,
    minimumSuccessRate = MIN_CALENDARIFIC_SUCCESS_RATE
) {
    const calendarific = dataset?.sources?.calendarific;
    if (
        !calendarific ||
        !calendarific.requests ||
        !Number.isFinite(calendarific.successRate)
    ) {
        throw new Error('Calendarific success metrics are missing. Existing KV dataset retained.');
    }
    if (calendarific.successRate >= minimumSuccessRate) return;

    const { expected, attempted, succeeded, failed, skipped } = calendarific.requests;
    const successPercent = (calendarific.successRate * 100).toFixed(1);
    const minimumPercent = (minimumSuccessRate * 100).toFixed(1);
    throw new Error(
        `Calendarific success rate ${successPercent}% (${succeeded}/${expected} expected; ` +
        `${attempted} attempted, ${failed} failed, ${skipped} skipped) is below the ` +
        `${minimumPercent}% publication threshold. Existing KV dataset retained.`
    );
}

async function buildHolidayDataset({
    fetchJson,
    apiKey = '',
    logger = console,
    years = getYearsToFetch()
} = {}) {
    if (typeof fetchJson !== 'function') {
        throw new TypeError('fetchJson must be provided');
    }

    const safeLogger = getLogger(logger);
    if (!apiKey) {
        safeLogger.warn('Warning: Calendarific API key not found. Skipping Calendarific holidays.');
    }

    const dataset = createDatasetSkeleton(apiKey);
    const calendarificStats = dataset.sources.calendarific.requests;
    const nationalBaselineCache = new Map();

    async function fetchTrackedCalendarific(request) {
        calendarificStats.expected += 1;
        if (!apiKey) {
            calendarificStats.skipped += 1;
            return [];
        }

        calendarificStats.attempted += 1;
        try {
            const holidays = await fetchCalendarificHolidays(
                fetchJson,
                apiKey,
                request,
                safeLogger
            );
            calendarificStats.succeeded += 1;
            return holidays;
        } catch (error) {
            calendarificStats.failed += 1;
            throw error;
        }
    }

    async function getCountryBaseline(countryCode, year) {
        const cacheKey = `${countryCode}-${year}`;
        if (!nationalBaselineCache.has(cacheKey)) {
            nationalBaselineCache.set(cacheKey, (async () => {
                let calendarificList = [];
                let tallyfyList = [];

                try {
                    calendarificList = await fetchTrackedCalendarific({
                        countryCode,
                        year,
                        types: CALENDARIFIC_NATIONAL_TYPES
                    });
                } catch (e) {
                    safeLogger.error(`Failed to fetch Calendarific holidays for ${countryCode} ${year}`);
                    calendarificList = [];
                }

                try {
                    tallyfyList = await fetchTallyfyHolidays(fetchJson, countryCode, year);
                } catch (e) {
                    tallyfyList = [];
                }

                const providerHolidays = mergeHolidayLists(calendarificList, tallyfyList);
                if (countryCode === US_NATIONAL_BASELINE.countryCode) {
                    return mergeHolidayLists(providerHolidays, getUsFederalHolidays(year));
                }
                return providerHolidays;
            })());
        }

        return nationalBaselineCache.get(cacheKey);
    }

    async function buildCountryLocation(location) {
        const yearsData = {};
        await mapWithConcurrency(YEAR_CONCURRENCY, years, async (year) => {
            yearsData[String(year)] = await getCountryBaseline(location.countryCode, year);
        });
        safeLogger.log(`Finished processing ${location.label} (${location.datasetKey})`);
        return [location.datasetKey, createLocationEntry(location, yearsData)];
    }

    async function buildUsStateLocation(location) {
        const yearsData = {};
        await mapWithConcurrency(YEAR_CONCURRENCY, years, async (year) => {
            const nationalBaseline = await getCountryBaseline(US_NATIONAL_BASELINE.countryCode, year);
            let overlayList = [];

            try {
                overlayList = await fetchTrackedCalendarific({
                    countryCode: location.countryCode,
                    year,
                    location: location.calendarificLocation,
                    types: CALENDARIFIC_LOCAL_TYPES
                });
            } catch (e) {
                safeLogger.error(`Failed to fetch Calendarific holidays for ${location.datasetKey} ${year}`);
                overlayList = [];
            }

            yearsData[String(year)] = mergeHolidayLists(overlayList, nationalBaseline);
        });
        safeLogger.log(`Finished processing ${location.label} (${location.datasetKey})`);
        return [location.datasetKey, createLocationEntry(location, yearsData)];
    }

    const countryEntries = await mapWithConcurrency(
        COUNTRY_LOCATION_CONCURRENCY,
        COUNTRY_LEVEL_DATASET_LOCATIONS,
        buildCountryLocation
    );

    countryEntries.forEach(([datasetKey, entry]) => {
        dataset.locations[datasetKey] = entry;
    });

    const stateEntries = await mapWithConcurrency(
        STATE_LOCATION_CONCURRENCY,
        US_STATE_DATASET_LOCATIONS,
        buildUsStateLocation
    );

    stateEntries.forEach(([datasetKey, entry]) => {
        dataset.locations[datasetKey] = entry;
    });

    dataset.sources.calendarific.successRate = calendarificStats.expected > 0
        ? calendarificStats.succeeded / calendarificStats.expected
        : 0;

    return dataset;
}

export {
    CALENDARIFIC_LOCAL_TYPES,
    CALENDARIFIC_NATIONAL_TYPES,
    CALENDARIFIC_URL,
    MIN_CALENDARIFIC_SUCCESS_RATE,
    TALLYFY_URL,
    YEARS_AHEAD,
    assertCalendarificPublicationSafe,
    buildHolidayDataset,
    fetchCalendarificHolidays,
    fetchTallyfyHolidays,
    getYearsToFetch,
    redactErrorMessage,
    redactUrl
};
