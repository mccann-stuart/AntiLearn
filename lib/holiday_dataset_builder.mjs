    ...
    async function getCountryBaseline(countryCode, year) {
        ...
        if (!nationalBaselineCache.has(cacheKey)) {
            nationalBaselineCache.set(cacheKey, (async () => {
                ...
                const calendarificPromise = fetchCalendarificHolidays(fetchJson, apiKey, {
                    countryCode,
                    year,
                    types: CALENDARIFIC_NATIONAL_TYPES
                }, safeLogger).catch(e => {
                ...
                const tallyfyPromise = fetchTallyfyHolidays(fetchJson, countryCode, year).catch(e => {
                    return [];
                });

                const [fetchedCalendarificList, fetchedTallyfyList] = await Promise.all([calendarificPromise, tallyfyPromise]);
                calendarificList = fetchedCalendarificList;
                tallyfyList = fetchedTallyfyList;
