...
## 2026-04-20 - Prevent Local Storage Memory Bloat / Client-Side DoS
...
## $(date +%Y-%m-%d) - Prevent State Leakage in Browser History
**Vulnerability:** When users loaded a shared plan via the `plan` query parameter, the application parsed the state but left the parameter in the browser's address bar.
**Learning:** Leaving sensitive state parameters in the address bar can lead to accidental state leakage if the user shares the URL, if they copy-paste it elsewhere, or via HTTP Referer headers.
**Prevention:** Always ensure URL parameters that represent transient state or sensitive tokens (like `SHARE_PARAM`) are stripped from the browser's address bar using `window.history.replaceState` immediately after they have been consumed.
