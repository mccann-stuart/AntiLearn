...
## 2026-05-19 - Accessible Context for Disabled Buttons on Touch Devices
**Learning:** When using `aria-disabled="true"` to keep buttons focusable so keyboard users can access `title` tooltips, touch-device users remain excluded because they cannot hover to trigger the tooltip. The tooltip information is essentially invisible to them.
**Action:** To ensure context from `title` attributes on buttons with `aria-disabled="true"` is accessible to touch-device users, add a click-event handler that triggers an `aria-live` toast notification displaying the tooltip text when the disabled button is tapped.
