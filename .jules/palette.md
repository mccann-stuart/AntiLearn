...
## 2026-04-19 - Explicit ARIA Roles for Dynamic Toasts
**Learning:** When dynamically creating toast notifications, assigning explicit ARIA roles (`role="alert"` for errors, `role="status"` for info/success) directly on the toast element ensures more reliable screen reader announcements than relying solely on the parent container's `aria-live` attribute.
**Action:** Always assign `role="alert"` to error notifications and `role="status"` to info/success notifications dynamically added to the DOM.
