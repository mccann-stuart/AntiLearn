...
function generateAllCandidates(year, allowance) {
    ...
    if (numWorkdays >= allowance) {
        totalCandidates = (numWorkdays - allowance + 1) * allowance + ((allowance - 1) * allowance) / 2;
    } else {
...
function updateDayNode(el, date, dateStr = null) {
    ...
    if (type === 'workday') {
        ...
        // Bolt Optimization: Use manual array lookups instead of Intl.DateTimeFormat overhead (~10x faster)
        const dateLabel = WEEKDAYS[date.getDay()] + ' ' + date.getDate() + ' ' + MONTHS_LONG[date.getMonth()];
